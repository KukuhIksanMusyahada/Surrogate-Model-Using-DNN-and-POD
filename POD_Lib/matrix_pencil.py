...
def compute_damping_coefficient(pitch,plunge):
        #load data

        time=np.arange(1,114)
        plunge_mp = MatrixPencil(t=time, x=plunge)
        pitch_mp = MatrixPencil(t=time, x=pitch)
        damping_coefficient = compare(pitchMode=pitch_mp, plungeMode=plunge_mp)
        return damping_coefficient

def search_all_damping(mach_min,mach_max,vf_min,vf_max,step_mach=0.01,step_vf=0.1):
  data=[]
  for mach in np.arange(mach_min,mach_max,step_mach):
    for vf in np.arange(vf_min,vf_max,step_vf):
      if mach<0.7:
        X_pi=X_pi1
        y_pi=y_pi1
        X_pl=X_pl1
        ...
        X_pl=X_pl3
        y_pl=y_pl3
      Pitch=POD_Predict(X_pi, y_pi, np.round(mach,2),np.round(vf,1),27,case='Pitch',Vf_type='Low')
      Plunge=POD_Predict(X_pl, y_pl,np.round(mach,2), np.round(vf,1),27,case='Plunge',Vf_type='Low')
      damping=compute_damping_coefficient(Pitch,Plunge)
      if abs(damping)>0.1:
        continue
      print(mach,vf,damping)
      data.append([mach,vf,damping])
